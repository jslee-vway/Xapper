...
- `xapper_drag` : 드래그 (요소→요소 / 요소+픽셀 오프셋 / 절대 화면 좌표). **시작 지점**이 스플리터·슬라이더·스크롤바 썸이면 그 컨트롤의 드래그 이벤트로 처리해 커서도 포커스도 건드리지 않는다 (목록에 스크롤바가 있다고 항목 드래그가 스크롤로 바뀌지 않는다). 그 외(항목 드롭·캔버스 요소 이동)도 기본은 대상 프로세스 안에서 몰아 커서·포커스를 건드리지 않으며, 그 경로를 설치할 수 없을 때만 실제 마우스 입력으로 폴백한다(응답에 어느 경로였는지 표시)
